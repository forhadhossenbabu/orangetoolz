...
export default function lanes(state = defaultState, action) {
  switch (action.type) {
    ...
    case actionTypes.MOVE_NOTE: {
      ...
      const sourceLane = state.filter(
        (lane) => lane.notes.indexOf(sourceId) !== -1
      )[0];
      const targetLane = state.filter(
        (lane) => lane.notes.indexOf(targetId) !== -1
      )[0];
    ...
    case actionTypes.MOVE_LANE: {
      ...
      const lanesWithoutSource = state.filter(
        (lane, index) => index !== sourceLaneIndex
      );
